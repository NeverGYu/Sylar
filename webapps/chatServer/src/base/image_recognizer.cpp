#include "../../include/base/image_recognizer.h"
...
{
    const std::string resolved_label_path =
        label_path.empty() ? chatserver::imageLabelPath() : label_path;
    ...
    LoadLabels(resolved_label_path);
}

void ImageRecognizer::LoadLabels(const std::string& label_path) {
    if (label_path.empty()) {
        throw std::runtime_error("CHAT_SERVER_IMAGE_LABELS is not set");
    }
    ...
    const std::string& label_path)
{
    ...
    throw std::runtime_error(
        "chat_server was built without OpenCV and ONNX Runtime support; image recognition is unavailable");
}

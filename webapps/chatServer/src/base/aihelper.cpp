#include "../../include/base/aihelper.h"
#include "../../include/base/chatserver_runtime.h"
...
std::string AIHelper::chat(int userId,std::string userName, std::string sessionId, std::string userQuestion, std::string modelType) {
    ...
    if (false == strategy->isMCPModel) {
        ...
        nlohmann::json payload = strategy->buildRequest(this->messages);

        //执行请求
        nlohmann::json response = executeCurl(payload);
    ...
    //说明支持MCP
    AIConfig config;
    if (!config.loadFromFile(chatserver::aiConfigPath())) {
        throw std::runtime_error("Failed to load AI tool config: " + chatserver::aiConfigPath());
    }
    ...
    nlohmann::json firstReq = strategy->buildRequest(this->messages);
    nlohmann::json firstResp = executeCurl(firstReq);
    ...
    nlohmann::json secondReq = strategy->buildRequest(messages);
    nlohmann::json secondResp = executeCurl(secondReq);
...
// 发送自定义请求体
nlohmann::json AIHelper::request(const nlohmann::json& payload) {
...
// 内部方法：执行 curl 请求
nlohmann::json AIHelper::executeCurl(const nlohmann::json& payload) {
#ifndef SYLAR_ENABLE_AI_CURL
    (void)payload;
    throw std::runtime_error("chat_server was built without curl support; AI chat is unavailable in this build");
#else
...
void AIHelper::pushMessageToMysql(int userId, const std::string& userName, bool is_user, const std::string& userInput,long long ms, std::string sessionId) {
    ...
    std::string safeSessionId = escapeString(sessionId);
...
#else
    try {
        sylar::db::MysqlUtil mysql;
        mysql.executeUpdate(sql);
    } catch (const std::exception& e) {
        std::cerr << "[AIHelper] Failed to persist chat message: " << e.what() << std::endl;
    }
#endif
}
